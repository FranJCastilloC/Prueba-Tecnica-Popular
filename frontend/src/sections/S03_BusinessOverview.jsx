...
export default function S03_BusinessOverview({ onNext, onPrev }) {
  ...
  const KPI_CARDS = [
    ...
    { value: fmtNum(KPIs.n_produs),                label: 'Productos',            note: '4 tipos, 10 variantes',              tone: 'violet',  delay: 0.25 },
  ]
  ...
  return (
    <motion.section className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        <InsightCard label="País líder"      value="Argentina" text={`${fmtUSD(HIGHLIGHTS.top_country_volume)} · ${HIGHLIGHTS.pareto80_clients} clientes top`} tone="neutral" />
        <InsightCard label="Mejor trimestre" value={HIGHLIGHTS.best_quarter} text="Pico de actividad trimestral" tone="positive" />
      </div>
